...
const PlanOption: React.FC<{
    ...
    <div onClick={onClick} className={`relative p-4 rounded-xl text-left cursor-pointer transition-all duration-200 ${isSelected ? 'border-2 border-blue-500 bg-blue-50' : 'border-2 border-gray-200 bg-gray-100/50 hover:border-gray-300'}`}>
        ...
        <div className="flex justify-between items-start">
            <div>
                ...
                <p className="text-gray-700 font-semibold mt-1">{price}</p>
                {subtext && <p className="text-sm text-gray-500">{subtext}</p>}
            </div>
            {savings && <p className="font-semibold text-blue-600 text-right">{savings}</p>}
        </div>
    </div>
);
  ...
  return (
    <div className="fixed inset-0 bg-white z-50 animate-fade-in flex flex-col">
            ...
            <div className="p-6 space-y-6">
                ...
                <div className="text-center">
                    ...
                    <header className="mt-4">
                        <h1 className="text-4xl font-bold text-gray-900">FitMind PRO</h1>
                        <p className="text-gray-500 mt-2">Desbloqueie todo o potencial da sua jornada.</p>
                    </header>
                </div>

                <div className="bg-gray-100/50 p-6 rounded-2xl text-left">
                ...
                <div className="space-y-4">
                    <PlanOption
                        title="Anual"
                        price="R$ 10,41 / mês"
                        subtext="cobrado R$ 124,90 por ano"
                        isPopular
                        savings="Economize 73%"
        ...
        <div className="flex-shrink-0 p-6 pt-4 bg-white border-t border-gray-100">
            <button onClick={() => onSubscribe(selectedPlan)} className="w-full bg-black text-white py-4 rounded-xl text-lg font-semibold active:scale-[0.98] transition-transform">
                Assinar Agora
            </button>
            <div className="text-center mt-4">
                <button className="text-gray-500 font-semibold text-sm">Restaurar Compras</button>
                <p className="text-xs text-gray-400 mt-2">O pagamento será cobrado na sua conta da App Store. A assinatura é renovada automaticamente, a menos que seja cancelada 24 horas antes do final do período.</p>
            </div>
        </div>
    </div>
  );
