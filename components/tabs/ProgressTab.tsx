...
const TabButton: React.FC<{ label: string; isActive: boolean; onClick: () => void; }> = ({ label, isActive, onClick }) => (
    <button onClick={onClick} className={`w-1/3 py-2 rounded-lg font-semibold transition-all text-base active:scale-[0.98] ${isActive ? 'bg-white shadow' : 'text-gray-500'}`}>
        {label}
    </button>
);
...
const SinglePhotoViewerModal: React.FC<{photo: ProgressPhoto, onClose: () => void, onDelete: (photo: ProgressPhoto) => void}> = ({ photo, onClose, onDelete }) => {
    return (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 animate-pop-in" onClick={onClose}>
            <div className="relative bg-white rounded-3xl p-4 shadow-xl w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
                <img 
                    src={photo.photo_url} 
                ...
                <button 
                    onClick={onClose} 
                    className="absolute top-3 right-3 bg-black/40 text-white w-8 h-8 rounded-full flex items-center justify-center hover:bg-black/60 transition-all"
                >
                 ...
                 <button 
                    ...
                    className="absolute top-3 left-3 bg-black/40 text-white w-8 h-8 rounded-full flex items-center justify-center hover:bg-red-500 transition-all"
...
const DeleteConfirmationModal: React.FC<{
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-6" onClick={onClose}>
        <div className="bg-white rounded-3xl p-6 w-full max-w-sm text-center animate-pop-in" onClick={(e) => e.stopPropagation()}>
            <div className="w-16 h-16 bg-red-100 text-red-500 rounded-2xl mx-auto flex items-center justify-center mb-4">
                <TrashIcon className="w-8 h-8"/>
            </div>
            <h2 className="text-2xl font-bold text-gray-900">Excluir Foto?</h2>
...
const ProgressView: React.FC<{ onShowProModal: (type: 'feature' | 'engagement', title?: string) => void; }> = ({ onShowProModal }) => {
  const { userData, weightHistory, setWeightHistory, progressPhotos, setProgressPhotos, updateStreak } = useAppContext();
  ...
  const DONUT_COLORS = ['#111827', '#e5e7eb'];

  return (
    <>
      <section className="grid grid-cols-2 gap-4">
        <div className="flex flex-col space-y-3">
            <div className="bg-white border border-gray-200/80 shadow-sm rounded-3xl aspect-square flex items-center justify-center p-2 group">
              ...
              {sortedPhotos.length > 1 && (
                  sortedPhotos.map((_, index) => (
                      <button 
                          ...
                          className={`w-2 h-2 rounded-full transition-all duration-300 ${currentPhotoIndex === index ? 'bg-black scale-125' : 'bg-gray-300 hover:bg-gray-400'}`}
                      />
                  ))
              )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
              <button
                  ...
                  className="w-full bg-gray-200 text-gray-800 font-semibold py-3 rounded-xl disabled:opacity-50 transition-transform active:scale-95"
              >
                  Comparar
              </button>
              <button
                  onClick={() => photoInputRef.current?.click()}
                  className="w-full bg-black text-white font-semibold py-3 rounded-xl transition-transform active:scale-95"
              >
                  Adicionar
              </button>
          </div>
        ...
        <div className="bg-gray-100/60 rounded-3xl p-5 flex flex-col justify-between">
            ...
            <div className="relative flex-grow flex items-center justify-center my-4 min-h-[150px]">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                    <span className="text-4xl font-bold tracking-tighter leading-tight text-gray-900">{currentWeight.toFixed(1)}</span>
                    <span className="text-lg text-gray-500 font-medium -mt-1">kg</span>
                </div>
            </div>
            <div className="border-t border-gray-200/80 pt-4 flex justify-between items-center text-center">
                <div>
                    <p className="text-sm text-gray-500">Meta</p>
                    <p className="font-semibold text-lg text-gray-900">{userData.targetWeight} kg</p>
                </div>
                <div>
                    <p className="text-sm text-gray-500">Faltam</p>
                    <p className="font-semibold text-lg text-gray-900">{(currentWeight - userData.targetWeight).toFixed(1)} kg</p>
                </div>
            </div>
        </div>
      </section>
      
       <section className="bg-gray-100/60 rounded-3xl p-5">
          <h2 className="text-xl font-bold text-gray-800 mb-4">Histórico de Peso</h2>
          <div className="space-y-3">
              {weightHistory.length > 0 ? (
                weightHistory.slice(0, 4).map(entry => (
                  <div key={entry.id} className="flex justify-between items-center text-gray-700 pb-3 border-b border-gray-200/80 last:border-b-0 last:pb-0">
                 ...
                 <p className="text-center text-gray-500 py-4">Nenhum registro de peso.</p>
              )}
          </div>
      </section>
      ...
      <section className="bg-gray-100/60 rounded-3xl p-5">
       ...
       <section className="bg-gray-100/60 rounded-3xl p-5">
          <div className="flex items-center gap-3 mb-4">
            <div className="bg-white p-2 rounded-full shadow-sm"><ScaleIcon className="w-5 h-5 text-gray-600"/></div>
...
const PhotoComparisonModal: React.FC<{
    ...
    return (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-white rounded-3xl w-full max-w-md max-h-[90vh] flex flex-col animate-pop-in" onClick={e => e.stopPropagation()}>
                {view === 'select' ? (
                    <>
                        <header className="flex-shrink-0 flex items-center justify-between p-4 border-b border-gray-200">
                            <div className="w-8"></div>
                            <h2 className="text-lg font-semibold text-gray-800">Selecione 2 Fotos</h2>
                        ...
                        <footer className="flex-shrink-0 p-4 border-t border-gray-200">
                            <button onClick={handleCompareClick} disabled={selectedPhotos.length !== 2} className="w-full bg-black text-white py-3 rounded-xl font-semibold disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors">
                    ...
                    <>
                        <header className="flex-shrink-0 flex items-center justify-between p-4 border-b border-gray-200">
                            <button onClick={handleBackClick} className="text-blue-600 hover:text-blue-800 font-semibold text-base px-2 py-1">Voltar</button>
                            <h2 className="text-lg font-semibold text-gray-800">Comparação</h2>
                            <button onClick={onClose} className="text-gray-400 hover:text-gray-700 p-1 rounded-full">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                            </button>
                        </header>
                        <main className="flex-grow overflow-y-auto p-4">
                             <div className="w-full text-center mb-4">
                                ...
                                <p className="text-sm text-gray-500">de progresso</p>
                            </div>
                            <div className="flex gap-4 w-full">
                                {selectedPhotos.map(photo => (
                                    <div key={photo.id} className="w-1/2">
                                        ...
                                        <p className="text-center text-gray-700 font-semibold mt-2">{formatDate(photo.date)}</p>
                                    </div>
                                ))}
                            </div>
                        </main>
