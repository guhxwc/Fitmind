...
const IntensityButton: React.FC<{
  ...
  onClick: () => void;
    ...
    const defaultStyle = "bg-gray-200/70 text-gray-700 hover:bg-gray-300";
    const colorMap = {
        'Leve': 'text-green-600 bg-green-100/80 border-green-300',
        'Moderado': 'text-yellow-600 bg-yellow-100/80 border-yellow-300',
        'Severo': 'text-red-600 bg-red-100/80 border-red-300'
    };
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-end justify-center">
      <div className="bg-white w-full max-w-md h-[90%] rounded-t-3xl p-6 flex flex-col animate-slide-up">
        <div className="flex-shrink-0 flex items-center justify-between mb-2">
          <h2 className="text-2xl font-bold text-gray-900">Efeitos Colaterais</h2>
        ...
        <p className="text-gray-500 mb-6 font-semibold">{date.toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long' })}</p>

        <div className="flex-grow overflow-y-auto space-y-6">
            <section>
                <h3 className="text-lg font-semibold text-gray-800 mb-2">Sintomas comuns</h3>
                    ...
                    {COMMON_EFFECTS.map(name => {
                        ...
                        return (
                             <button
                                ...
                                className={`px-4 py-2 rounded-full font-semibold border-2 transition-colors ${isSelected ? 'bg-black text-white border-black' : 'bg-gray-100/80 border-gray-100/80 hover:border-gray-300'}`}
                             >
                                 {name}
                             </button>
                        )
            ...
            {effects.length > 0 && (
                <section className="space-y-4">
                    <h3 className="text-lg font-semibold text-gray-800">Intensidade</h3>
                    {effects.map(effect => (
                        <div key={effect.name} className="bg-gray-100/60 p-4 rounded-xl animate-fade-in">
                            <p className="font-bold text-gray-900 mb-3">{effect.name}</p>
                            <div className="grid grid-cols-3 gap-2 bg-white p-1 rounded-xl">
            ...
            <section>
                <h3 className="text-lg font-semibold text-gray-800 mb-2">Notas Adicionais</h3>
                <textarea
                    value={notes}
                    onChange={(e) => setNotes(e.target.value)}
                    placeholder="Algum detalhe importante? (opcional)"
                    className="w-full h-24 bg-gray-100/60 rounded-xl p-3 text-gray-800 focus:outline-none focus:ring-2 focus:ring-black"
                />
            </section>
        </div>

        <div className="mt-auto pt-6">
            <button onClick={handleSave} className="w-full bg-black text-white py-4 rounded-xl text-lg font-semibold">
                Salvar Registros
            </button>
        </div>
      </div>
