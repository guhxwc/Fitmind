...
async function startServer() {
  ...
  // Universal helper to call Groq API
  async function callGroqAPI(model: string, contents: any, config: any) {
  ...
  app.post("/api/gemini", async (req, res) => {
    try {
      ...
      const response = await callGroqAPI(model, contents, config);
      return res.status(200).json(response);
    } catch (error: any) {
      console.error("Groq-proxied API Error:", error);
